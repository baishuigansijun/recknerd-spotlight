...
<Image
    src={image7}
    alt="Room with paintings on the wall"
    // 用静态导入的图片时可以省略宽高（Next 会用文件元数据），保留也没问题
    // width={1200}
    // height={400}
    priority
...
- **Lightweight prints (less than 10 lbs / 4.5 kg):** regular nails or picture hooks are fine.
- **Medium works (10–30 lbs / 4.5–13.5 kg):** use wall anchors or heavy-duty hooks.
- **Heavy pieces (more than 30 lbs / 13.5 kg):** opt for French cleats, metal brackets, or professional installation.
